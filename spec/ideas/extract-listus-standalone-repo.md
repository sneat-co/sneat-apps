...
- Renaming calendarius or contactus to the new convention — listus only for now
...
| Might-be-true  | The singular `extension-` naming convention is the right long-term pattern for all extensions (calendarius, contactus, etc.), not just listus.                                                                      | Document it in the sneat-libs README and revisit when migrating the next extension.                                                                      |
...
- **Registry for the publish step:** public npm (matches the calendarius precedent, fits the public AGPL repo) vs GitHub Packages (token-authed, non-public). (Working assumption: public npm.)
