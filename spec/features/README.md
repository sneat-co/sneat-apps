...
| [extension-permission-management-ui](extension-permission-management-ui/README.md) | In Review | A settings screen where the user reviews installed extensions (name, icon, origin), sees the scopes currently granted to each, revokes an individual scope, and removes an extension entirely (revoking all its scopes, deleting its registration, and dropping its origin from the dynamic frame-src allowlist). |
