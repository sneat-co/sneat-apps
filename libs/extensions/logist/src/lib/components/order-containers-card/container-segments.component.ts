...
export class ContainerSegmentsComponent implements OnChanges {
		...
		if (changes['order'] || changes['container']) {
			...
			this.routeRows = this.containerSegments?.map((segment, i) => {
				...
				const from = fromPoint
					? `${fromPoint.location?.title} @ ${fromPoint?.counterparty?.title}`
					: 'pointID:' + segment.from.shippingPointID;
				const to = toPoint
					? `${toPoint.location?.title} @ ${toPoint?.counterparty?.title}`
					: 'pointID:' + segment.to.shippingPointID;
				return {
					i: i + 1,
					segment,
					from,
			...
			if (
				...
				!this.containerSegments?.some(
					(s) =>
						s.from?.shippingPointID ===
							this.selectedSegment?.from?.shippingPointID &&
						s.to?.shippingPointID === this.selectedSegment?.to?.shippingPointID,
				)
			) {
	...
	protected segmentID(_: number, segment: IContainerSegment): string {
		return (
			segment.containerID +
			'-' +
			segment.from.shippingPointID +
			'-' +
			segment.to.shippingPointID
		);
	}
	protected readonly rowSelected = (row: unknown) => {
		this.selectedSegment = (
			row as { getData: () => { segment: IContainerSegment } }
		).getData().segment;
	};
}
