...
import { ILogistOrderContext, IOrderContainer, IOrderShippingPoint, ShippingPointTask } from '../../dto';
...
export class ShippingPointsSelectorComponent implements OnChanges {
	...
	setShippingPoints(): void {
		...
		this.order?.dto?.shippingPoints?.forEach(sp => {
			...
			if (!selected?.dirty) {
				...
				if (container) {
					tasks = this.order?.dto?.containerPoints?.find(cp => cp.containerID === container.id && cp.shippingPointID === sp.id)?.tasks || [];
				}
	...
	protected addShippingPoint(): void {
		...
		this.newShippingPointService.openNewShippingPointDialog({
			order: this.order,
			container: this.container,
		}).catch(this.errorLogger.logErrorHandler('Failed to open new shipping point dialog'));
	}

	protected checkboxChanged(event: Event, shippingPointID: string, task: ShippingPointTask): void {
		const ce = event as CustomEvent;
		console.log('checkboxChanged', ce);
		const selected = this.tasksByShippingPoint[shippingPointID] || {tasks: [], dirty: false};
		...
		if (tasks) {
			const container = this.container;
			if (container) {
				const containerTasks = this.order?.dto?.containerPoints?.find(cp => cp.containerID === container.id && cp.shippingPointID === shippingPointID)?.tasks || [];
				if ([...tasks].sort().join(';') === [...containerTasks].sort().join(';')) {
	...
	protected onContactChanged(contact: IContactContext): void {
		...
		if (!this.newShippingPoints?.find(sp => sp.location?.contactID === contact.id)) {
			this.newShippingPoints = [...(this.newShippingPoints || []), {
				id: '',
				tasks: [],
				status: 'pending',
				counterparty: {
					contactID: contact.parentContact?.id || '',
					title: contact?.parentContact?.brief?.title || contact.parentContact?.id || '',
				},
				location: {
