...
export abstract class AssetBasePage extends CommuneBasePage implements OnInit {
	...
	protected constructor(
		defaultBackPage: 'real-estates' | CommuneTopPage.assets | CommuneTopPage.asset | CommuneTopPage.vehicles,
	...
	protected setAssetDto(assetDto?: IAssetDto): void {
		...
		if (assetDto.communeId && this.communeRealId && assetDto.communeId !== this.communeRealId) {
	...
	ngOnInit(): void {
		...
		this.route.queryParamMap.pipe(first())
			.subscribe(params => {
				const assetId = params.get('id') || params.get('asset') || (this.asset && this.asset.dto.id);
				if (!assetId) {
					throw new Error('!assetId');
				}
				this.assetService.watchById(assetId)
					.subscribe(asset => {
						console.log(`AssetPage.assetService.watchItemById(${assetId}) =>`, asset);
