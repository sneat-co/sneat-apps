...
// 	ISneatReadwriteTx,
// 	SneatAppSchema,
// } from '../../../models/db-schemas-by-app/sneat-app-schema';
// import {
// 	IListusReadwriteTx,
// 	IListusStoreProvider,
// 	ListusKind,
// } from '../../../models/db-schemas-by-app/listus-app-schema';
...
// 	affectsCommune = true,
// ): ListusKind[] {
// 	const kinds: ListusKind[] = affectsCommune
// 		? [ListKind, CommuneKind]
// 		: [ListKind];
// 	if (item && item.id) {
...
// 	}
//
// 	private static addItemInfoToListDto(
// 		listDto: IListDto,
// 		item: IListItemInfo,
...
// 		}
// 		const existingItem =
// 			items && items.find((v) => !v.id && eq(v.title, item.title));
// 		if (existingItem) {
// 			return {
// 				listDto,
// 				success: false,
// 				message: `Duplicate item: ${item.title}`,
// 			};
// 		}
// 		if (!items) {
// 			items = [];
...
// 	}
//
// 	private static updateListICommuneDto(
// 		listDto: IListDto,
// 		communeDto: ICommuneDto,
// 	): boolean {
// 		let changed = false;
...
// 	}
//
// 	private static deleteItemInfoFromListDto(
// 		listDto: IListDto,
// 		item: IListItemInfo,
// 		communeDto: ICommuneDto,
...
//
// 		console.log(
// 			'ListusService.deleteItemInfoFromListDto() => itemsToDelete:',
// 			itemsToDelete,
// 		);
//
...
// 		}
// 		changed =
// 			ListusDbService.updateListICommuneDto(listDto, communeDto) || changed;
// 		return { dto: listDto, changed };
// 	}
//
// 	public reorderListItems(
// 		params: IListItemsCommandParams,
// 		reorderItems: ReorderListItemsWorker,
// 	): Observable<IListDto> {
...
// 					!dto.items ||
// 					!list.dto.items ||
// 					dto.items.length !== list.dto.items.length
// 				) {
...
// 			const o = !movie.id
// 				? of(undefined)
// 				: this.movieService.getById(movie.id, tx);
// 			return o.pipe(
// 				mergeMap((movieDto) => {
// 					if (!movieDto) {
...
// 					}
// 					return forkJoin(
// 						movieDto.listIds.map(
// 							(listId) => this.listService.getById(listId, tx), // TODO: update movie info in list DTO.
// 						),
// 					).pipe(mapTo(movieDto));
// 				}),
// 			);
...
// 		}
// 		console.log(
// 			`ListusService.deleteListItem(list{shortId: ${list.shortId}, id: ${
// 				list.dto.id
// 			}, item=${items[0].id || items[0].title})`,
// 		);
...
// 			}
// 			return forkJoin([
// 				this.communeObserver(tx, commune).pipe(
// 					tap((v) => {
// 						console.log('commune:', v);
// 					}),
// 				),
// 				// tslint:disable-next-line:no-non-null-assertion
// 				this.deleteListItemRecord(tx, id).pipe(
// 					tap((v) => {
// 						console.log('item:', v);
// 					}),
// 				),
...
// 					this.updateListDto(
// 						tx,
// 						{ dto: list.dto, shortId: list.shortId },
// 						communeDto,
// 						(listDto) =>
// 							ListusDbService.deleteItemInfoFromListDto(
// 								listDto,
// 								items[0],
// 								communeDto,
// 							),
...
// 				}),
// 				mergeMap((v) => {
// 					const deleteListItemMutation = mutation<{
// 						list: string;
// 						item: string;
// 					}>('delete-list-item');
// 					if (!list.dto.id) {
...
// 					const $v: Observable<IListDto> = this.mutationLogger
// 						.recordMutation(tx, m)
// 						.pipe(mapTo(v));
// 					return $v;
// 				}),
...
// 				return this.updateListDto(tx, list, communeDto, (listDto) => {
// 					listItemResult = ListusDbService.addItemInfoToListDto(
// 						listDto,
// 						items[0],
// 						communeDto,
// 					);
// 					return {
// 						dto: listItemResult.listDto,
// 						changed: listItemResult.success,
// 					};
// 				}).pipe(
...
// 	}
//
// 	private deleteListItemRecord(
// 		tx: IListusReadwriteTx,
// 		id: RxRecordKey,
// 	): Observable<IListItemDto | undefined> {
// 		return id ? this.listItemService.deleteRecord(tx, id) : of(undefined);
// 	}
//
// 	private updateListDto(
// 		tx: ISneatReadwriteTx,
// 		list: IList,
// 		communeDto: ICommuneDto,
// 		worker: RxRecordUpdater<IListDto>,
// 	): Observable<IListDto> {
// 		if (!list) {
// 			throw new Error(
// 				'ListusService.updateListDto(): list parameter is missing',
// 			);
// 		}
// 		if (!list.dto) {
// 			throw new Error(
// 				`ListusService.updateListDto(): list.dto is missing, shortId=${list.shortId}`,
// 			);
// 		}
// 		console.log(
// 			`ListusService.updateListDto(list{shortId:${list.shortId}, id:${
// 				list.dto && list.dto.id
// 			})`,
// 		);
//
...
// 				.updateRecord(tx, list.dto.id, worker)
// 				.pipe(afterListRecordProcessed);
// 		}
// 		const { dto, changed } = worker(list.dto);
// 		if (changed) {
// 			return this.listService.add(dto, tx).pipe(afterListRecordProcessed);
// 		}
// 		return of(list.dto);
// 	}
//
// 	private updateCommuneCounter(
// 		tx: IRxReadwriteTransaction<SneatAppSchema>,
// 		list: IList,
// 		communeDto: ICommuneDto,
// 	): Observable<IListDto> {
// 		console.log(
// 			`ListusService.updateCommuneCounter(list.shortId=${list.shortId}, list.dto.id=${list.dto.id})`,
// 		);
...
// 		let changed = false;
// 		const listGroup =
// 			communeDto.listGroups &&
// 			communeDto.listGroups.find((lg) => lg.type === listDto.type);
// 		let listInfo =
// 			listGroup &&
// 			listGroup.lists &&
// 			listGroup.lists.find((l) => eq(l.id, listDto.id));
// 		if (!listInfo) {
// 			listInfo =
// 				listGroup &&
// 				listGroup.lists &&
// 				listGroup.lists.find((l) => isListInfoMatchesListDto(l, list));
// 			if (listInfo) {
...
// 			return this.communeService.put(communeDto, tx).pipe(mapTo(listDto));
// 		}
// 		return of(listDto);
// 	}
//
// 	private communeObserver(
// 		tx: ISneatReadwriteTx,
// 		commune: Commune,
// 	): Observable<ICommuneDto> {
// 		return commune.id
// 			? this.communeService.mustGetById(commune.id, tx)
// 			: commune.shortId
// 			? this.communeService
// 					.materializeVirtualCommune(tx, commune.dto, commune.shortId)
// 					.pipe(map((v) => v.communeDto))
// 			: throwError('!commune.id && !commune.shortID');
// 	}
//
// 	public copyListItems(
// 		fromListId: string,
// 		toListId: string,
...
// 		console.log(
// 			`ListusService.copyListItems(${fromListId}, ${toListId}):`,
// 			listItems,
// 		);
// 		// if (!fromListId) {
...
// 			communeInfo = userDto.communes
// 				? UserModel.getCommuneInfoById(shortCommuneId, userDto.communes)
// 				: undefined;
...
// 		}
// 		return this.readwriteTx(kinds, (tx: IListusReadwriteTx) => {
// 			const commune$ =
// 				needToCreateCommune || !communeInfo
// 					? this.communeService.createStandardCommune(
// 							tx,
// 							shortCommuneId as CommuneType,
// 							userId,
// 					  )
// 					: this.communeService
// 							.mustGetById(communeInfo.id || '', tx)
...
// 					if (shortListId) {
// 						const listInfo = CommuneModel.getListInfoByShortId(
// 							commune,
// 							shortListId,
// 						);
// 						if (listInfo) {
...
// 					} else {
// 						result = throwError(
// 							'Not implemented copy of items for communes without short ID',
// 						);
// 					}
// 					return result;
// 				}),
// 				mergeMap(({ commune, toList }) => {
// 					if (!toList) {
...
// 								(item) =>
// 									!!toList.items &&
// 									!toList.items.some((existing) => eq(existing.id, item.id)),
// 							)
// 							.map((item) => ({ ...item })),
// 					);
// 					console.log('ready to save target list:', toList);
// 					const $saveList = toList.id
// 						? this.listService.put(toList, tx)
// 						: this.listService.add(toList, tx);
