...
export class DataGridComponent implements AfterViewInit, OnChanges, OnDestroy {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    try {
      if (changes.data && this.data && this.columns || changes.columns && this.columns) {
        this.drawTable();
      }
    } catch (ex) {
  ...
  private drawTable(): void {
    ...
    console.log('drawTable()', 'columns:', this.columns, 'groupBy:', this.groupBy, 'data:', this.data);
    try {
  ...
  private createTabulatorGrid(): void {
    ...
    this.tabulatorOptions = {
      ...
      columns: this.columns.map(c => {
        const col: any = { // TODO(help-wanted): Use strongly typed Tabulator col def
          field: c.field,
          title: c.title || c.field,
          headerTooltip: () => `${c.colName || c.title || c.field}: ${c.dbType}`,
        };
