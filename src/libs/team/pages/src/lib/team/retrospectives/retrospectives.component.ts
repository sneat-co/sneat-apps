...
export class RetrospectivesComponent {
  ...
  navigateToCurrentRetro(): void {
    ...
    if (!this.team) {
    	this.errorLogger.logError('Can not navigate to retro without having team context');
    	return;
	}
    const activeRetroId = this.team?.data?.active?.retrospective?.id;
    try {
      this.navService.navigateToRetrospective(this.navController,
		  this.team, activeRetroId || 'upcoming');
    } catch (e) {
  ...
  retroCount(itemType: RetroItemType): number {
    ...
    return (userId ? this.team?.data?.upcomingRetro?.itemsByUserAndType?.[userId]?.[itemType] : 0) || 0;
  }
}
