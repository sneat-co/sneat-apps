type Portion = ['year' | 'day' | 'hour' | 'minute' | 'second', 'min' | 'sec' | undefined, (seconds: number) => number];

const y = 31536000, d = 86400, h = 3600;
...
export const secondsToStr = (s: number): string | undefined => {
	...
	return a.map(v => v[2] + ' ' + (a.length === 1 || !v[1] ? v[0] : v[1]) + (v[2] === 1 ? '' : 's')).join(' ');
}
